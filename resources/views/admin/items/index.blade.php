...
@endsection
@section('contents')
    <<<<<<< HEAD <h1 class="mt-3 pb-2" style="border-bottom: 2px solid #dedede">Items Home
        ...
        <div class="row">
                            ...
                            <select class="form-select" name="parent" id="parent_cat">
                                ...
                                @foreach ($cats as $cat)
                                    ...
                                    @foreach ($cat->children as $cc)
                                        <option value="{{ $cc->id }}">{{ $cat->name }} - {{ $cc->name }}
                                        </option>
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="card card-body">
                            <form action="/admin/items/categories/store" method="POST">
                                ...
                                <div class="input-group">
                                    ...
                                    <label class="input-group-text" for="cat_name">Name</label>
                                    <input type="text" class="form-control" name="name" id="cat_name">
                                    >>>>>>> 3edb9cd4cd3eba6e9580dab2e41a08d020d06fb5
                                </div>
                ...
                <div class="row">
                    <div class="col col-12 collapse" id="addItemForm">
                        <div class=" card card-body">
                            ...
                            <form action="{{ route('store-new-product') }}" method="POST" accept="[]"
                                    ...
                                    <select class="form-select" name="category_id" id="categorey">
                                        ...
                                        @foreach ($centrals as $cat)
                                            @foreach ($cat->children as $child)
                                                <option value="{{ $child->id }}">{{ $cat->parent->name }}
                                                    -{{ $cat->name }} -
                                                    {{ $child->name }}
                                                </option>
                                            @endforeach
            ...
            <<<<<<< HEAD <!-- end -->
                        ...
                        <fieldset class="mt-4 mx-0 mb-0">
                            <legend>Display Product List &nbsp; &nbsp;
                                <a class=" ms-3" data-bs-toggle="collapse" data-bs-target="#addItemForm"
                                    aria-expanded="false" aria-controls="addItemForm"><i data-bs-toggle="tooltip"
                                        title="Add New Item" class="fa fa-plus"></i></a>
                            </legend>
                ...
                <script>
                    ...
                    === = <
                    /div> >>>
                    >>> > 3 edb9cd4cd3eba6e9580dab2e41a08d020d06fb5
                        ...
                        fieldset class = "mt-4 mx-0 mb-0" >
                        <
                        legend > Root & nbsp; & nbsp; <
                    a class = " ms-3"
                    data - bs - toggle = "collapse"
                    data - bs - target = "#addItemCategoryForm"
                    aria - expanded = "false"
                    aria - controls = "addItemCategoryForm" > < i data - bs - toggle = "tooltip"
                        ...
                        @if ($item->children)
                            ...
                            ol >
                                @foreach ($item->children as $child)
                                    <
                                    li id = "cats"
                                    data - ajax - url = "{{ route('product.filter') }}"
                                    data - ajax - token = "{{ csrf_token() }}" >
                                        <
                                        input type = "checkbox"
                                    id = "item_{{ $child->id }}"
                    ...
                    fieldset class = "mt-4 mx-0 mb-0" >
                    <
                    legend > Display Product List & nbsp; & nbsp; <
                    a class = " ms-3"
                    data - bs - toggle = "collapse"
                    data - bs - target = "#addItemForm"
                    aria - expanded = "false"
                    aria - controls = "addItemForm" > < i data - bs - toggle = "tooltip"
                    title = "Add New Item"
                    class = "fa fa-plus" > < /i></a >
                    <
                    /legend>
                    ...
                    << << < HEAD
                        ...
                        @foreach ($products as $product)
                            ...
                            alt = "img" >
                                ...
                                h5 class = " mb-1" >
                                @if ($product->parent_cat && $product->parent_cat->parent)
                                    {{ $product->parent_cat->parent->name }} & gt;
                                    {{ $product->parent_cat->name }}
                                @else
                                    {{ $product->name }}
                                @endif <
