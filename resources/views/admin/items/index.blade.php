      ...
      <ol id="tree" class="mt-0">
        ...
        <li>
          ...
          <ol>
            ...
            <li  id="cats" data-ajax-url="{{ route('product.filter') }}" data-ajax-token="{{ csrf_token() }}">
              <input type="checkbox" id="item_{{ $child->id }}" checked />
              <label class="category-item" data-element-id="0" for="item_{{ $child->id }}">{{ $child->name }}</label>
              @if ($child->children)
              <ol>
                @foreach ($child->children as $grandChild)
                <li  data-element-id="item_{{ $child->id }}" class="category-item">
                  <label >
                    {{ $grandChild->name }}
                  </label>
                </li>
