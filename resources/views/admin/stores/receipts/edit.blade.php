...
@section('contents')
    <fieldset class="mt-5 mb-4 mx-0 mb-0 shadow-sm">
        ...
        <div class="row">
            <div
                class=" card-body col {{ $errors->has('reference') || $errors->has('brief') || $errors->has('notes') ? 'col-9 show' : 'col-12' }}">
                <form action="{{ route('update-receipt-info') }}" method="POST">
                    <input type="hidden" name="id" value="{{ $receipt->id }}">
                    @csrf
                    <div class="input-group  mb-2">
                        <label class="input-group-text text-muted" for="reference"><i
                                class="fa fa-tag  px-2"></i>Reference</label>
                        <input type="text" class="form-control " name="reference"
                            value="{{ old('reference', $receipt) }}" />
                        <label class="input-group-text text-muted" for="reference_type"><i class="fa fa-tags  px-2"></i>
                            Reference Type</label>
                        <select class="form-select  " name="reference_type" id="reference_type">
                            @foreach ($reference_types as $key => $value)
                                <option value="{{ $key }}"
                                    {{ $receipt->reference_type == $key ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                        <label class="input-group-text  text-muted" for="admin_id"><i
                                class="fa fa-user  px-2"></i>Representative</label>
                        <select class="form-select  " name="admin_id" id="admin_id">
                            @foreach ($admins as $admin)
                                <option hidden>{{ $receipt->admin->userName }}</option>
                                <option value="{{ $admin->id }}"
                                    {{ $receipt->admin_id == $admin->id ? 'selected' : '' }}>{{ $admin->userName }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="input-group  mt-2">
                        ...
                        <input type="text" class="form-control " name="brief" id="brief"
                            value="{{ old('brief', $receipt) }}">
                    </div>
                    <div class="input-group  mt-2">
                        <label class="input-group-text text-muted" for="notes"><i
                                class="fa fa-sticky-note  px-2"></i>Notes</label>
                        <input type="text" class="form-control" name="notes" id="notes"
                            value="{{ old('notes', $receipt) }}">
            ...
            <div class="col p-0 pb-3 card {{ $errors->has('reference') || $errors->has('brief') || $errors->has('notes') ? 'col-3 d-inline-block' : 'd-none' }}"
